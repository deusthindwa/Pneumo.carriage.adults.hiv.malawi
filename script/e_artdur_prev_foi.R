...
#subset for a correct dataset
medium = pcvpa.mod %>% select(nvtcarr, agegp, surv, artdur) %>% filter(artdur == 1) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_medium = gam(nvtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps"), family = binomial(link = "cloglog"), data = medium)
medium$fit = predict.gam(model_medium, type = "response", se.fit = TRUE)$fit
medium$fit_lci = model_medium$family$linkinv(predict.gam(model_medium, type = "link", se.fit = TRUE)$fit - (2 * predict.gam(model_medium, type = "link", se.fit = TRUE)$se.fit))
medium$fit_uci = model_medium$family$linkinv(predict.gam(model_medium, type = "link", se.fit = TRUE)$fit + (2 * predict.gam(model_medium, type = "link", se.fit = TRUE)$se.fit))

#join observed and predicted datasets for agegp
medium1 <- left_join(left_join(medium %>% group_by(agegp) %>% tally() %>% rename(Tot = n), 
medium %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
medium %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>% mutate(sex="Female") %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:8], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[9:16])

#join observed and predicted datasets for survey number
medium2 <- left_join(left_join(medium %>% group_by(surv) %>% tally() %>% rename(Tot = n), 
medium %>% filter(nvtcarr == 1) %>% group_by(surv) %>% tally() %>% rename(Pos = n)), 
medium %>% filter(nvtcarr != 0) %>% group_by(surv) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>% mutate(sex="Female") %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:7], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[8:14])

#----------------------------------------------------------------------------------

#fit model to obtain predictions of FOI
model_medium = scam(nvtcarr ~ s(agegp, bs="mdcv") + s(surv, bs="mdcv"), family = binomial(link = "cloglog"), data = medium)
medium$foi <- ((-derivative.scam(model_medium, smooth.number = 1, deriv = 1)$d * model_medium$fitted.values) + (1/42*model_medium$fitted.values))/(1-model_medium$fitted.values)

#join observed and predicted datasets for agegp
medium3 <- left_join(left_join(medium %>% group_by(agegp) %>% tally() %>% rename(Tot = n), 
medium %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
medium %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(foi = mean(foi)) %>%
ungroup()) %>% select(foi)

#join observed and predicted datasets for survey number
medium4 <- left_join(left_join(medium %>% group_by(surv) %>% tally() %>% rename(Tot = n), 
medium %>% filter(nvtcarr == 1) %>% group_by(surv) %>% tally() %>% rename(Pos = n)), 
medium %>% filter(nvtcarr != 0) %>% group_by(surv) %>% summarise(foi = mean(foi)) %>%
ungroup()) %>% select(foi)

#----------------------------------------------------------------------------------

#plot prevalence curves
C <- ggplot(data = cbind(medium1, medium3)) +
  geom_point(aes(x = agegp, y = obs, size = Pos), shape = 1) +
  geom_errorbar(aes(agegp, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  ...
  scale_y_continuous("", sec.axis = sec_axis(~. * 0.15, name = ""), limits = c(0, 0.75)) + 
  labs(title = "ART 2-6y", x = "Age,y") + 
  theme_bw() + 
  theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) +
  theme(plot.title = element_text(size = 14), axis.title.x = element_text(size = 10), axis.title.y = element_text(size = 10)) +
  theme(legend.position = "none")

D <- ggplot(data = cbind(medium2, medium4)) +
  geom_point(aes(x = surv, y = Pos/Tot, size = Pos), shape = 1) +
  geom_errorbar(aes(surv, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
...
#subset for a correct dataset
longer = pcvpa.mod %>% select(nvtcarr, agegp, surv, artdur) %>% filter(artdur == 2) %>% group_by(surv)
...
#plot prevalence curves
E <- ggplot(data = cbind(longer1, longer3)) +
  geom_point(aes(x = agegp, y = obs, size = Pos), shape = 1) +
  geom_errorbar(aes(agegp, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
...
F <- ggplot(data = cbind(longer2, longer4)) +
  geom_point(aes(x = surv, y = Pos/Tot, size = Pos), shape = 1) +
  geom_errorbar(aes(surv, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
...
#fit model to obtain predictions of FOI
model_shorter = scam(vtcarr ~ s(agegp, bs="mdcv") + s(surv, bs="mdcv"), family = binomial(link = "cloglog"), data = shorter)
shorter$foi <- ((-derivative.scam(model_shorter, smooth.number = 1, deriv = 1)$d * model_shorter$fitted.values) + (1/42*model_shorter$fitted.values))/(1-model_shorter$fitted.values)
...
#plot prevalence curves
G <- ggplot(data = cbind(shorter1, shorter3)) +
  geom_point(aes(x = agegp, y = obs, size = Pos), shape = 1) +
  geom_errorbar(aes(agegp, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = agegp, y = fit), size = 1, color = "red") +
  geom_ribbon(aes(x = agegp, y = fit, ymin = fit_lci, ymax = fit_uci), alpha = 0.2, fill = "red", color = "gray") +
...
H <- ggplot(data = cbind(shorter2, shorter4)) +
  geom_point(aes(x = surv, y = Pos/Tot, size = Pos), shape = 1) +
  geom_errorbar(aes(surv, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = surv, y = fit), size = 1, color = "red") +
  geom_point(aes(x = surv, y = foi/0.15), size = 1, shape = 18, color = "red") +
...
#subset for a correct dataset
medium = pcvpa.mod %>% select(vtcarr, agegp, surv, artdur) %>% filter(artdur == 1) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_medium = gam(vtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps"), family = binomial(link = "cloglog"), data = medium)
medium$fit = predict.gam(model_medium, type = "response", se.fit = TRUE)$fit
medium$fit_lci = model_medium$family$linkinv(predict.gam(model_medium, type = "link", se.fit = TRUE)$fit - (2 * predict.gam(model_medium, type = "link", se.fit = TRUE)$se.fit))
medium$fit_uci = model_medium$family$linkinv(predict.gam(model_medium, type = "link", se.fit = TRUE)$fit + (2 * predict.gam(model_medium, type = "link", se.fit = TRUE)$se.fit))
...
#join observed and predicted datasets for agegp
medium1 <- left_join(left_join(medium %>% group_by(agegp) %>% tally() %>% rename(Tot = n), 
medium %>% filter(vtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
medium %>% filter(vtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>% mutate(sex="Female") %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:8], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[9:16])

#join observed and predicted datasets for survey number
medium2 <- left_join(left_join(medium %>% group_by(surv) %>% tally() %>% rename(Tot = n), 
medium %>% filter(vtcarr == 1) %>% group_by(surv) %>% tally() %>% rename(Pos = n)), 
medium %>% filter(vtcarr != 0) %>% group_by(surv) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>% mutate(sex="Female") %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:7], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[8:14])

#----------------------------------------------------------------------------------

#fit model to obtain predictions of FOI
model_medium = scam(vtcarr ~ s(agegp, bs="mdcv") + s(surv, bs="mdcv"), family = binomial(link = "cloglog"), data = medium)
medium$foi <- ((-derivative.scam(model_medium, smooth.number = 1, deriv = 1)$d * model_medium$fitted.values) + (1/42*model_medium$fitted.values))/(1-model_medium$fitted.values)

#join observed and predicted datasets for agegp
medium3 <- left_join(left_join(medium %>% group_by(agegp) %>% tally() %>% rename(Tot = n), 
medium %>% filter(vtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
medium %>% filter(vtcarr != 0) %>% group_by(agegp) %>% summarise(foi = mean(foi)) %>%
ungroup()) %>% select(foi)

#join observed and predicted datasets for survey number
medium4 <- left_join(left_join(medium %>% group_by(surv) %>% tally() %>% rename(Tot = n), 
medium %>% filter(vtcarr == 1) %>% group_by(surv) %>% tally() %>% rename(Pos = n)), 
medium %>% filter(vtcarr != 0) %>% group_by(surv) %>% summarise(foi = mean(foi)) %>%
ungroup()) %>% select(foi)

#----------------------------------------------------------------------------------

#plot prevalence curves
I <- ggplot(data = cbind(medium1, medium3)) +
  geom_point(aes(x = agegp, y = obs, size = Pos), shape = 1) +
  geom_errorbar(aes(agegp, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = agegp, y = fit), size = 1, color = "blue") +
  geom_ribbon(aes(x = agegp, y = fit, ymin = fit_lci, ymax = fit_uci), alpha = 0.2, fill = "blue", color = "gray") +
...
J <- ggplot(data = cbind(medium2, medium4)) +
  geom_point(aes(x = surv, y = Pos/Tot, size = Pos), shape = 1) +
  geom_errorbar(aes(surv, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = surv, y = fit), size = 1, color = "blue") +
  geom_point(aes(x = surv, y = foi/0.15), size = 1, shape = 18, color = "blue") +
...
#subset for a correct dataset
longer = pcvpa.mod %>% select(vtcarr, agegp, surv, artdur) %>% filter(artdur == 2) %>% group_by(surv)
...
#plot prevalence curves
K <- ggplot(data = cbind(longer1, longer3)) +
  geom_point(aes(x = agegp, y = obs, size = Pos), shape = 1) +
  geom_errorbar(aes(agegp, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = agegp, y = fit), size = 1, color = "darkgreen") +
  geom_ribbon(aes(x = agegp, y = fit, ymin = fit_lci, ymax = fit_uci), alpha = 0.2, fill = "green", color = "gray") +
...
L <- ggplot(data = cbind(longer2, longer4)) +
  geom_point(aes(x = surv, y = Pos/Tot, size = Pos), shape = 1) +
  geom_errorbar(aes(surv, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = surv, y = fit), size = 1, color = "darkgreen") +
  geom_point(aes(x = surv, y = foi/0.15), size = 1, shape = 18, color = "darkgreen") +
