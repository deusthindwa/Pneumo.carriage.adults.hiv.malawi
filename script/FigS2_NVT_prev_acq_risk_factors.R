...
female = filter(pcvpa.mod, sex == 1) %>% select(nvtcarr, age, year)

#fit model to individual trajectories & obtain predictions and 95%CI
model_female = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, sex == 1), na.action = na.exclude)
...
female %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
female %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
female %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
female %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
#subset for a dataset to store model estimates
male = filter(pcvpa.mod, sex == 2) %>% select(nvtcarr, age, year)

#fit model to individual trajectories & obtain predictions and 95%CI
model_male = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, sex == 2), na.action = na.exclude)
...
male %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
male %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
male %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
male %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
#subset for a dataset to store model estimates
lses = filter(pcvpa.mod, sescat == 1) %>% select(nvtcarr, age, year)

#fit model to individual trajectories & obtain predictions and 95%CI
model_lses = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, sescat == 1), na.action = na.exclude)
...
lses %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
lses %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
lses %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
lses %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
#subset for a dataset to store model estimates
hses = filter(pcvpa.mod, sescat == 2) %>% select(nvtcarr, age, year)

#fit model to individual trajectories & obtain predictions and 95%CI
model_hses = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, sescat == 2), na.action = na.exclude)
...
hses %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
hses %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
hses %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
hses %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
#subset for a dataset to store model estimates
sart = filter(pcvpa.mod, artdur == 1) %>% select(nvtcarr, age, year)

#fit model to individual trajectories & obtain predictions and 95%CI
model_sart = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, artdur == 1), na.action = na.exclude)
...
sart %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
sart %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
sart %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
sart %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
#subset for a dataset to store model estimates
lart = filter(pcvpa.mod, artdur == 2) %>% select(nvtcarr, age, year)

#fit model to individual trajectories & obtain predictions and 95%CI
model_lart = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, artdur == 2), na.action = na.exclude)
...
lart %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
lart %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:4], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[5:8], foi = fit/dur, foi_lci = fit_lci/dur, foi_uci = fit_uci/dur, rf = "ART ≥3y")
...
lart %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
lart %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:5], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[6:10], foi = fit/dur, foi_lci = fit_lci/dur, foi_uci = fit_uci/dur, rf = "ART ≥3y")

#======================================================================================

#subset for a dataset to store model estimates
nochild5 = filter(pcvpa.mod, nochild5 == 1) %>% select(nvtcarr, age, year)

#fit model to individual trajectories & obtain predictions and 95%CI
model_nochild5 = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, nochild5 == 1), na.action = na.exclude)
...
nochild5 %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
nochild5 %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
nochild5 %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
nochild5 %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
#subset for a dataset to store model estimates
yeschild5 = filter(pcvpa.mod, nochild5 == 2) %>% select(nvtcarr, age, year)

#fit model to individual trajectories & obtain predictions and 95%CI
model_yeschild5 = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, nochild5 == 2), na.action = na.exclude)
...
yeschild5 %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
yeschild5 %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
yeschild5 %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
yeschild5 %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
A <- pcvpa_age %>% mutate(rff = factor(rf, levels=c('Male', 'Female', 'Low SES', 'Middle/High SES', 'ART <3y', 'ART ≥3y', 'Living with <5y child', 'Living without <5y child'))) %>%
  ...
  scale_y_continuous("NVT carriage prevalence", sec.axis = sec_axis(~. * 0.1, name = "Daily NVT carriage acquisition"), limits = c(0, 0.65)) + 
...
ggsave(here("output", "FigS2_NVT_prev_acq_risk_factors.tiff"),
       plot = (A / B ),
