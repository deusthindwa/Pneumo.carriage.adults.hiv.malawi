...
male = filter(pcvpa.mod, sex == 1) %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_male = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + nochild5 + seas, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, sex == 1), na.action = na.exclude)
...
A <- ggplot(male_age) +
  ...
  scale_y_continuous("NVT carriage prevalence", sec.axis = sec_axis(~. * 0.1, name = ""), limits = c(0, 0.7)) + 
...
#subset for a dataset to store model estimates
female = filter(pcvpa.mod, sex == 0) %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_female = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + nochild5 + seas, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, sex == 0), na.action = na.exclude)
...
D <- ggplot(female_year) +
  ...
  scale_y_continuous("", sec.axis = sec_axis(~. * 0.1, name = "Daily carriage acquisition"), limits = c(0, 0.7)) + 
  labs(title = "", x = "Year") +
  theme_bw() +
  theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) +
  theme(plot.title = element_text(size = 14), axis.title.x = element_text(size = 10), axis.title.y = element_text(size = 10)) +
...
#subset for a dataset to store model estimates
lses = filter(pcvpa.mod, sescat == 0) %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_lses = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + sex + nochild5 + seas, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, sescat == 0), na.action = na.exclude)
...
#subset for a dataset to store model estimates
hses = filter(pcvpa.mod, sescat == 1) %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_hses = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + sex + nochild5 + seas, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, sescat == 1), na.action = na.exclude)
...
ggsave(here("output", "FigS3_NVT_prev_acq_sex_ses.tiff"),
