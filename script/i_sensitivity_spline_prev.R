...
model_crude = gam(nvtcarr ~ te(agegp, bs="cs") + te(surv, bs="cs") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fitcs = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit

model_crude = gam(nvtcarr ~ te(agegp, bs="tp") + te(surv, bs="tp") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fittp = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit

model_crude = gam(nvtcarr ~ te(agegp, bs="cr") + te(surv, bs="cr") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
...
#fit different models & obtain predictions and 95%CI
model_crude = gam(vtcarr ~ te(agegp, bs="cs") + te(surv, bs="cs") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fitcs = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit

model_crude = gam(vtcarr ~ te(agegp, bs="tp") + te(surv, bs="tp") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fittp = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit

model_crude = gam(vtcarr ~ te(agegp, bs="cr") + te(surv, bs="cr") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fitcr = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit
...
model1_crude = gam(nvtcarr ~ te(agegp, bs="ps") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fit1 = predict.gam(model1_crude, type = "response", se.fit = TRUE)$fit

model2_crude = gam(nvtcarr ~ te(surv, bs="ps") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fit2 = predict.gam(model2_crude, type = "response", se.fit = TRUE)$fit

model3_crude = gam(nvtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fit3 = predict.gam(model3_crude, type = "response", se.fit = TRUE)$fit

model4_crude = gam(nvtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps") + ti(agegp, surv, bs="ps") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
...
#fit different model formulations & obtain predictions and 95%CI
model1_crude = gam(vtcarr ~ te(agegp, bs="ps") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fit1 = predict.gam(model1_crude, type = "response", se.fit = TRUE)$fit

model2_crude = gam(vtcarr ~ te(surv, bs="ps") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fit2 = predict.gam(model2_crude, type = "response", se.fit = TRUE)$fit

model3_crude = gam(vtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fit3 = predict.gam(model3_crude, type = "response", se.fit = TRUE)$fit

model4_crude = gam(vtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps") + ti(agegp, surv, bs="ps") + sex + nochild5, family = binomial(link = "cloglog"), data = crude)
crude$fit4 = predict.gam(model4_crude, type = "response", se.fit = TRUE)$fit
...
H <- ggplot(data = crude2) +
  geom_point(aes(x = surv, y = obs, size = Pos), shape = 1) +
  geom_errorbar(aes(surv, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = surv, y = fit1, colour = "Model1: constant + ∑(confounders) + te(age)"), size = 0.8) +
  geom_line(aes(x = surv, y = fit2, colour = "Model2: constant + ∑(confounders) + te(time)"), size = 0.8) +
  geom_line(aes(x = surv, y = fit3, colour = "Model3: constant + ∑(confounders) + te(age) + te(time)"), size = 0.8) +
  geom_line(aes(x = surv, y = fit4, colour = "Model4: constant + ∑(confounders) + te(age) + te(time) + ti(age, time)"), size = 0.8) +
  scale_color_discrete(breaks=c("Model1: constant + ∑(confounders) + te(age)", 
                                "Model2: constant + ∑(confounders) + te(time)", 
                                "Model3: constant + ∑(confounders) + te(age) + te(time)", 
                                "Model4: constant + ∑(confounders) + te(age) + te(time) + ti(age, time)")) +
  guides(color=guide_legend(title="Model formulation"), size=FALSE) +
  labs(title = "", x = "Survey number", y = "") +
  theme_bw() +
  ylim(0, 0.4) +
...
ggsave(here("output", "SFig4_sens_spline_model.tiff"),
