...
lowses = pcvpa.mod %>% select(nvtcarr, agegp, surv, sescat) %>% filter(sescat == 0) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_lowses = gam(nvtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps"), family = binomial(link = "cloglog"), data = lowses)
...
model_lowses = scam(nvtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpi"), family = binomial(link = "cloglog"), data = lowses)
...
#subset for a correct dataset
midses = pcvpa.mod %>% select(nvtcarr, agegp, surv, sescat) %>% filter(sescat == 1) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_midses = gam(nvtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps"), family = binomial(link = "cloglog"), data = midses)
...
#fit model to obtain predictions of FOI
model_midses = scam(nvtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpd"), family = binomial(link = "cloglog"), data = midses)
...
#subset for a correct dataset
highses = pcvpa.mod %>% select(nvtcarr, agegp, surv, sescat) %>% filter(sescat == 2) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_highses = gam(nvtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps"), family = binomial(link = "cloglog"), data = highses)
...
#fit model to obtain predictions of FOI
model_highses = scam(nvtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpi"), family = binomial(link = "cloglog"), data = highses)
...
#subset for a correct dataset
lowses = pcvpa.mod %>% select(vtcarr, agegp, surv, sescat) %>% filter(sescat == 0) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_lowses = gam(vtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps"), family = binomial(link = "cloglog"), data = lowses)
lowses$fit = predict.gam(model_lowses, type = "response", se.fit = TRUE)$fit
lowses$fit_lci = model_lowses$family$linkinv(predict.gam(model_lowses, type = "link", se.fit = TRUE)$fit - (2 * predict.gam(model_lowses, type = "link", se.fit = TRUE)$se.fit))
lowses$fit_uci = model_lowses$family$linkinv(predict.gam(model_lowses, type = "link", se.fit = TRUE)$fit + (2 * predict.gam(model_lowses, type = "link", se.fit = TRUE)$se.fit))
...
#fit model to obtain predictions of FOI
model_lowses = scam(vtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpd"), family = binomial(link = "cloglog"), data = lowses)
lowses$foi <- ((-derivative.scam(model_lowses, smooth.number = 1, deriv = 1)$d * model_lowses$fitted.values) + (1/42*model_lowses$fitted.values))/(1-model_lowses$fitted.values)
...
#subset for a correct dataset
midses = pcvpa.mod %>% select(vtcarr, agegp, surv, sescat) %>% filter(sescat == 1) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_midses = gam(vtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps"), family = binomial(link = "cloglog"), data = midses)
midses$fit = predict.gam(model_midses, type = "response", se.fit = TRUE)$fit
midses$fit_lci = model_midses$family$linkinv(predict.gam(model_midses, type = "link", se.fit = TRUE)$fit - (2 * predict.gam(model_midses, type = "link", se.fit = TRUE)$se.fit))
midses$fit_uci = model_midses$family$linkinv(predict.gam(model_midses, type = "link", se.fit = TRUE)$fit + (2 * predict.gam(model_midses, type = "link", se.fit = TRUE)$se.fit))
...
#fit model to obtain predictions of FOI
model_midses = scam(vtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpd"), family = binomial(link = "cloglog"), data = midses)
midses$foi <- ((-derivative.scam(model_midses, smooth.number = 1, deriv = 1)$d * model_midses$fitted.values) + (1/42*model_midses$fitted.values))/(1-model_midses$fitted.values)
...
#subset for a correct dataset
highses = pcvpa.mod %>% select(vtcarr, agegp, surv, sescat) %>% filter(sescat == 2) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_highses = gam(vtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps"), family = binomial(link = "cloglog"), data = highses)
highses$fit = predict.gam(model_highses, type = "response", se.fit = TRUE)$fit
highses$fit_lci = model_highses$family$linkinv(predict.gam(model_highses, type = "link", se.fit = TRUE)$fit - (2 * predict.gam(model_highses, type = "link", se.fit = TRUE)$se.fit))
highses$fit_uci = model_highses$family$linkinv(predict.gam(model_highses, type = "link", se.fit = TRUE)$fit + (2 * predict.gam(model_highses, type = "link", se.fit = TRUE)$se.fit))
...
#fit model to obtain predictions of FOI
model_highses = scam(vtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpd"), family = binomial(link = "cloglog"), data = highses)
highses$foi <- ((-derivative.scam(model_highses, smooth.number = 1, deriv = 1)$d * model_highses$fitted.values) + (1/42*model_highses$fitted.values))/(1-model_highses$fitted.values)
...
ggsave(here("output", "Fig5_ses_prev_foi.tiff"),
