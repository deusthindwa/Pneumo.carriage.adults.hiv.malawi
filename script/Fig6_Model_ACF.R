...
J <- ggAcf(overall_nvt$residuals, type = "correlation", lag.max = 5, ylim = c(-0.10,0.10), size = 1) + 
...
L <- ggAcf(male_nvt$residuals, type = "correlation", lag.max = 5, color = "red", ylim = c(-0.10,0.10), size = 1) + 
...
N <- ggAcf(hses_nvt$residuals, type = "correlation", lag.max = 5, ylim = c(-0.10,0.10), size = 1) + 
...
P <- ggAcf(lart_nvt$residuals, type = "correlation", lag.max = 5, color = "red", ylim = c(-0.10,0.10), size = 1) + 
  labs(title = "(NVT) ART ≥3y", x = "Lag year", y = "ACF") + 
  theme_bw()  
...
ggsave(here("output", "Fig6_Model_ACF.tiff"),
