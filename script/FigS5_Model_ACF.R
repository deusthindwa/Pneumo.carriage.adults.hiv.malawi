...
Q <- ggAcf(yeschild5_nvt$residuals, type = "correlation", lag.max = 5,  color = "red", ylim = c(-0.10,0.10), size = 1) + 
  labs(title = "(NVT) With <5y child", x = "Lag year", y = "") + 
  theme_bw()

R <- ggAcf(nochild5_nvt$residuals, type = "correlation", lag.max = 5,  color = "red", ylim = c(-0.10,0.10), size = 1) + 
  labs(title = "(NVT) Without <5y child", x = "Lag year", y = "") + 
  theme_bw()  
...
ggsave(here("output", "FigS5_Model_ACF.tiff"),
