...
female = filter(pcvpa.mod) %>% 
  select(carr, age, year, sex) %>%
  mutate(sex = as_factor(if_else(sex == 1, "Female", "Male")))

#fit model to individual trajectories & obtain predictions and 95%CI
model_female = gam(carr ~ te(age, bs="ps") + te(year, bs="ps") + te(age, bs="ps", by = sex) + te(year, bs="ps", by = sex), family = binomial(link = "cloglog"), data = female, na.action = na.exclude)
