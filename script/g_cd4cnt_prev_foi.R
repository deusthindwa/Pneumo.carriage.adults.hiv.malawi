...
lowcd4cnt = pcvpa.mod %>% select(nvtcarr, agegp, surv, cd4cnt, sex) %>% filter(cd4cnt == 0) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_lowcd4cnt = gam(nvtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps") + sex, family = binomial(link = "cloglog"), data = lowcd4cnt)
...
model_lowcd4cnt = scam(nvtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpd") + sex, family = binomial(link = "cloglog"), data = lowcd4cnt)
...
#subset for a correct dataset
highcd4cnt = pcvpa.mod %>% select(nvtcarr, agegp, surv, cd4cnt, sex) %>% filter(cd4cnt == 1) %>% group_by(surv)
highcd4cnt$nvtcarr <-  if_else(highcd4cnt$surv == 1L & highcd4cnt$agegp == 34L, 1L, highcd4cnt$nvtcarr)

#fit model & obtain predictions and 95%CI
model_highcd4cnt = gam(nvtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps") + sex, family = binomial(link = "cloglog"), data = highcd4cnt)
...
#fit model to obtain predictions of FOI
model_highcd4cnt = scam(nvtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpd") + sex, family = binomial(link = "cloglog"), data = highcd4cnt)
...
#subset for a correct dataset
lowcd4cnt = pcvpa.mod %>% select(vtcarr, agegp, surv, cd4cnt, sex) %>% filter(cd4cnt == 0) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_lowcd4cnt = gam(vtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps") + sex, family = binomial(link = "cloglog"), data = lowcd4cnt)
lowcd4cnt$fit = predict.gam(model_lowcd4cnt, type = "response", se.fit = TRUE)$fit
lowcd4cnt$fit_lci = model_lowcd4cnt$family$linkinv(predict.gam(model_lowcd4cnt, type = "link", se.fit = TRUE)$fit - (2 * predict.gam(model_lowcd4cnt, type = "link", se.fit = TRUE)$se.fit))
lowcd4cnt$fit_uci = model_lowcd4cnt$family$linkinv(predict.gam(model_lowcd4cnt, type = "link", se.fit = TRUE)$fit + (2 * predict.gam(model_lowcd4cnt, type = "link", se.fit = TRUE)$se.fit))
...
#subset for a correct dataset
lowcd4cnt = pcvpa.mod %>% select(vtcarr, agegp, surv, cd4cnt, sex) %>% filter(cd4cnt == 0 & surv != 7) %>% group_by(surv)
...
#fit model to obtain predictions of FOI
model_lowcd4cnt = scam(vtcarr ~ s(agegp, bs="mdcx") + s(surv, bs="mpd") + sex, family = binomial(link = "cloglog"), data = lowcd4cnt)
lowcd4cnt$foi <- ((-derivative.scam(model_lowcd4cnt, deriv = 1)$d * model_lowcd4cnt$fitted.values) + (1/42*model_lowcd4cnt$fitted.values))/(1-model_lowcd4cnt$fitted.values)
...
#subset for a correct dataset
highcd4cnt = pcvpa.mod %>% select(vtcarr, agegp, surv, cd4cnt, sex) %>% filter(cd4cnt == 1) %>% group_by(surv)

#fit model & obtain predictions and 95%CI
model_highcd4cnt = gam(vtcarr ~ te(agegp, bs="ps") + te(surv, bs="ps") + sex, family = binomial(link = "cloglog"), data = highcd4cnt)
highcd4cnt$fit = predict.gam(model_highcd4cnt, type = "response", se.fit = TRUE)$fit
highcd4cnt$fit_lci = model_highcd4cnt$family$linkinv(predict.gam(model_highcd4cnt, type = "link", se.fit = TRUE)$fit - (2 * predict.gam(model_highcd4cnt, type = "link", se.fit = TRUE)$se.fit))
highcd4cnt$fit_uci = model_highcd4cnt$family$linkinv(predict.gam(model_highcd4cnt, type = "link", se.fit = TRUE)$fit + (2 * predict.gam(model_highcd4cnt, type = "link", se.fit = TRUE)$se.fit))
...
#subset for a correct dataset
highcd4cnt = pcvpa.mod %>% select(vtcarr, agegp, surv, cd4cnt, sex) %>% filter(cd4cnt == 1 & surv !=7) %>% group_by(surv)
...
#fit model to obtain predictions of FOI
model_highcd4cnt = scam(vtcarr ~ s(agegp, bs="mpd") + s(surv, bs="mpd") + sex, family = binomial(link = "cloglog"), data = highcd4cnt)
highcd4cnt$foi <- ((-derivative.scam(model_highcd4cnt, deriv = 1)$d * model_highcd4cnt$fitted.values) + (1/42*model_highcd4cnt$fitted.values))/(1-model_highcd4cnt$fitted.values)
...
ggsave(here("output", "Fig6_cd4+_prev_foi.tiff"),
