...
A <- ggplot(data = crude1) +
  ...
  geom_line(aes(x = agegp, y = fitcs, colour = "Natural cubic spline", group = 1), size = 0.8) +
  geom_line(aes(x = agegp, y = fittp, colour = "Thin plate regression spline", group = 1), size = 0.8) +
  geom_line(aes(x = agegp, y = fitps, colour = "P-spline", group = 1), size = 0.8) +
  geom_line(aes(x = agegp, y = fitcr, colour = "Cubic regression spline", group = 1), size = 0.8) +
...
B <- ggplot(data = crude2) +
  ...
  geom_line(aes(x = year, y = fitcs, color = "Natural cubic spline", group = 1), size = 0.8) +
  geom_line(aes(x = year, y = fittp, color = "Thin plate regression spline", group = 1), size = 0.8) +
  geom_line(aes(x = year, y = fitps, color = "P-spline", group = 1), size = 0.8) +
  geom_line(aes(x = year, y = fitcr, color = "Cubic regression spline", group = 1), size = 0.8) +
  scale_color_discrete(breaks=c("Natural cubic spline", "Thin plate regression spline", "P-spline", "Cubic regression spline")) +
...
#fit different models & obtain predictions and 95%CI
model_crude = gam(nvtcarr ~ te(age, bs="cs") + te(year, bs="cs") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = pcvpa.mod)
crude$fitcs = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit

model_crude = gam(nvtcarr ~ te(age, bs="tp") + te(year, bs="tp") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = pcvpa.mod)
crude$fittp = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit

model_crude = gam(nvtcarr ~ te(age, bs="cr") + te(year, bs="cr") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = pcvpa.mod)
crude$fitcr = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit
...
crude %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fitcs = mean(fitcs), fittp = mean(fittp), fitcr = mean(fitcr), fitps = mean(fitps)) %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:5], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[6:10])
...
C <- ggplot(data = crude1) +
  ...
  scale_color_discrete(breaks=c("Natural cubic spline", "Thin plate regression spline", "P-spline", "Cubic regression spline")) +
  guides(color=guide_legend(title="Types of splines fitted"), size = FALSE) +
  labs(title = "Overall NVT carriage", x = "Age in years", y = "") +
  theme_bw() +
  ylim(0, 0.4) +
  theme(axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) +
  theme(plot.title = element_text(size = 14), axis.title.x = element_text(size = 10), axis.title.y = element_text(size = 10)) +
...
D <- ggplot(data = crude2) +
  geom_point(aes(x = year, y = Pos/Tot, size = Pos), shape = 1) +
  geom_errorbar(aes(year, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = year, y = fitcs, color = "Natural (cubic) spline"), size = 0.8) +
  geom_line(aes(x = year, y = fittp, color = "Thin plate regression spline"), size = 0.8) +
  geom_line(aes(x = year, y = fitps, color = "P-spline (B-spline plus penalisation)                   "), size = 0.8) +
  geom_line(aes(x = year, y = fitcr, color = "Cubic regression spline"), size = 0.8) +
...
E <- ggplot(data = crude1) +
  geom_point(aes(x = agegp, y = obs, size = Pos), shape = 1) +
  geom_errorbar(aes(agegp, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = agegp, y = fit1, colour = "Model1", group = 1), size = 0.8) +
  geom_line(aes(x = agegp, y = fit2, colour = "Model2", group = 1), size = 0.8) +
  geom_line(aes(x = agegp, y = fit3, colour = "Model3", group = 1), size = 0.8) +
  geom_line(aes(x = agegp, y = fit4, colour = "Model4", group = 1), size = 0.8) +
...
F <- ggplot(data = crude2) +
  ...
  geom_errorbar(aes(year, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = year, y = fit1, colour = "Model1"), size = 0.8) +
  geom_line(aes(x = year, y = fit2, colour = "Model2"), size = 0.8) +
  geom_line(aes(x = year, y = fit3, colour = "Model3"), size = 0.8) +
  geom_line(aes(x = year, y = fit4, colour = "Model4"), size = 0.8) +
  scale_color_discrete(breaks=c("Model1", "Model2", "Model3", "Model4")) +
  guides(color=guide_legend(title="Model formulation"), size = FALSE) +
  labs(title = "", x = "Survey year", y = "") +
  theme_bw() +
...
#fit different model formulations & obtain predictions and 95%CI
model1_crude = gam(nvtcarr ~ te(age, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = pcvpa.mod)
crude$fit1 = predict.gam(model1_crude, type = "response", se.fit = TRUE)$fit

model2_crude = gam(nvtcarr ~ te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = pcvpa.mod)
crude$fit2 = predict.gam(model2_crude, type = "response", se.fit = TRUE)$fit

model3_crude = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = pcvpa.mod)
crude$fit3 = predict.gam(model3_crude, type = "response", se.fit = TRUE)$fit

model4_crude = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + ti(age, year, bs="ps") + seas + sex + artdur + nochild5 + sescat, family = binomial(link = "cloglog"), data = pcvpa.mod)
crude$fit4 = predict.gam(model4_crude, type = "response", se.fit = TRUE)$fit
...
crude2 <- left_join(left_join(crude %>% group_by(year) %>% tally() %>% rename(Tot = n), 
crude %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
crude %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit1 = mean(fit1), fit2 = mean(fit2), fit3 = mean(fit3), fit4 = mean(fit4)) %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:5], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[6:10])
...
H <- ggplot(data = crude2) +
  geom_point(aes(x = year, y = obs, size = Pos), shape = 1) +
  geom_errorbar(aes(year, ymin = obs_lci, ymax = obs_uci), width = 0, size = 0.3) +
  geom_line(aes(x = year, y = fit1, colour = "Model1: constant + ∑(risk factors) + te(age)"), size = 0.8) +
  geom_line(aes(x = year, y = fit2, colour = "Model2: constant + ∑(risk factors) + te(time)"), size = 0.8) +
  geom_line(aes(x = year, y = fit3, colour = "Model3: constant + ∑(risk factors) + te(age) + te(time)"), size = 0.8) +
  geom_line(aes(x = year, y = fit4, colour = "Model4: constant + ∑(risk factors) + te(age) + te(time) + ti(age, time)"), size = 0.8) +
  scale_color_discrete(breaks=c("Model1: constant + ∑(risk factors) + te(age)", 
                                "Model2: constant + ∑(risk factors) + te(time)", 
                                "Model3: constant + ∑(risk factors) + te(age) + te(time)", 
                                "Model4: constant + ∑(risk factors) + te(age) + te(time) + ti(age, time)")) +
  guides(color=guide_legend(title="Model formulation"), size=FALSE) +
  labs(title = "", x = "Survey year", y = "") +
  theme_bw() +
  ylim(0, 0.4) +
...
ggsave(here("output", "SFig6_Model_Selection.tiff"),
