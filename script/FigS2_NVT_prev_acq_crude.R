...
crude = pcvpa.mod %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_crude = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + sex + nochild5 + seas, family = binomial(link = "cloglog"), data = pcvpa.mod, na.action = na.exclude)
...
A <- ggplot(crude_age) +
  ...
  scale_y_continuous("Carriage prevalence", sec.axis = sec_axis(~. * 0.1, name = ""), limits = c(0, 0.65)) + 
...
#subset for a dataset to store model estimates
crude = pcvpa.mod %>% select(nvtcarr1, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_crude = gam(nvtcarr1 ~ te(age, bs="ps") + te(year, bs="ps") + sex + nochild5 + seas, family = binomial(link = "cloglog"), data = pcvpa.mod, na.action = na.exclude)
crude$fit = predict.gam(model_crude, type = "response", se.fit = TRUE)$fit
crude$fit_lci = model_crude$family$linkinv(predict.gam(model_crude, type = "link", se.fit = TRUE)$fit - (2 * predict.gam(model_crude, type = "link", se.fit = TRUE)$se.fit))
crude$fit_uci = model_crude$family$linkinv(predict.gam(model_crude, type = "link", se.fit = TRUE)$fit + (2 * predict.gam(model_crude, type = "link", se.fit = TRUE)$se.fit))
...
ggsave(here("output", "FigS2_NVT_prev_acq_crude.tiff"),
       plot = (A | B | C | D),
