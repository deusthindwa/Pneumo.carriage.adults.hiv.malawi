...
sart = filter(pcvpa.mod, artdur == 0) %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_sart = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + nochild5 + seas, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, artdur == 0), na.action = na.exclude)
...
#subset for a dataset to store model estimates
lart = filter(pcvpa.mod, artdur == 1) %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_lart = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + nochild5 + seas, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, artdur == 1), na.action = na.exclude)
...
#subset for a dataset to store model estimates
lcd4 = filter(pcvpa.mod, cd4cnt == 0) %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_lcd4 = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + sex + nochild5 + seas, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, cd4cnt == 0), na.action = na.exclude)
...
lcd4 %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
lcd4 %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:4], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[5:8], foi = fit/42, foi_lci = fit_lci/42, foi_uci = fit_uci/42)
...
lcd4 %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
lcd4 %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
...
#subset for a dataset to store model estimates
hcd4 = filter(pcvpa.mod, cd4cnt == 1) %>% select(nvtcarr, age, year, sex, nochild5, seas)

#fit model to individual trajectories & obtain predictions and 95%CI
model_hcd4 = gam(nvtcarr ~ te(age, bs="ps") + te(year, bs="ps") + sex + nochild5 + seas, family = binomial(link = "cloglog"), data = filter(pcvpa.mod, cd4cnt == 1), na.action = na.exclude)
...
hcd4 %>% filter(nvtcarr == 1) %>% group_by(agegp) %>% tally() %>% rename(Pos = n)), 
hcd4 %>% filter(nvtcarr != 0) %>% group_by(agegp) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:4], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[5:8], foi = fit/42, foi_lci = fit_lci/42, foi_uci = fit_uci/42)
...
hcd4 %>% filter(nvtcarr == 1) %>% group_by(year) %>% tally() %>% rename(Pos = n)), 
hcd4 %>% filter(nvtcarr != 0) %>% group_by(year) %>% summarise(fit = mean(fit), fit_lci = mean(fit_lci), fit_uci = mean(fit_uci)) %>%
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:5], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[6:10], foi = fit/42, foi_lci = fit_lci/42, foi_uci = fit_uci/42)
...
ggsave(here("output", "FigS4_NVT_prev_acq_art_cd4.tiff"),
