...
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:4], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[5:8], status = "VT carriage")
...
ungroup()) %>% mutate(obs = Pos/Tot, obs_lci = exactci(Pos, Tot, 0.95)$conf.int[1:5], obs_uci = exactci(Pos, Tot, 0.95)$conf.int[6:10], status = "VT carriage")
...
A <- rbind(crude_age1, crude_age2, crude_age3) %>%
  mutate(status = factor(status, levels = c('Overall carriage', 'VT carriage', '(VT-st3) carriage'))) %>%
  ggplot() + 
  geom_point(aes(x = agegp, y = obs, size = Tot, color = status), shape = 1, position=position_dodge(width=0.05)) + 
...
B <- rbind(crude_year1, crude_year2, crude_year3) %>%
  mutate(status = factor(status, levels = c('Overall carriage', 'VT carriage', '(VT-st3) carriage'))) %>%
  ggplot() + 
  geom_point(aes(x = year, y = obs, size = Tot, color = status), shape = 1, position=position_dodge(width=0.05)) + 
  ...
  guides(color = guide_legend(title = "Serotype groups"), size = guide_legend(title = "Total samples"), group = "none", fill = "none") +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=1))
